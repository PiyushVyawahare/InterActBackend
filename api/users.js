...
router.post("/", async (req, res) => {
  try {
    ...
    if (is_duplicate) {
      if (is_duplicate.is_verified)
        return res
          .status(400)
          .send({ message: "User Already registered with us..." });
      else {
        const flag1 = generateAndMailOTP(is_duplicate, title);
        if (flag1)
          return res.status(200).send({
            message:
              "You are already registered. Please verify your email... OTP sent to your registered email",
          });
      }
    }
    ...
    if (user_details.profile_picture) {
      ...
      const data = await aws.uploadToS3(
        user_details.user_name + "_profilePicture",
        buffer
      );
      if (data)
    ...
    if (flag)
      return res
        .status(200)
        .send({ message: "Email was sent to your registered email" });
    return res.status(400).send({ message: "Something went wrong!" });
  } catch (error) {
    console.log("Error occured in register User, Error:", error);
  }
});
...
const generateAndMailOTP = async (user_details, title) => {
  ...
  const template = await emailTemplate(otp, user_details.user_name);
...
router.post("/verify", async (req, res) => {
  try {
    ...
    if (!otp || !otp._id) {
      return res
        .status(400)
        .send({ message: "User not found or email is already verified." });
    }
    ...
    return res
      .status(400)
      .send({ message: "Couldn't verify user, please enter valid OTP" });
  } catch (error) {
    console.log("Error occured in verify User, Error:", error);
  }
});
...
router.post("/signin", async (req, res) => {
  try {
    ...
    if (!user_details.is_verified)
      return res
        .status(404)
        .send({ message: "To sign in, please verify your email first." });
    ...
    const expiry = new Date().getTime() + 100 * 60000;
    ...
    console.log("Error occured in SignIn, Error:", error);
  }
});
...
router.get("/search", async (req, res) => {
  try {
    ...
    const users_details = await users
      .find({
        $or: [
          {
    ...
    console.log("Error occured in Search rooms, Error:", error);
  }
});
